...
parent_ic50 = pd.read_excel('./data/192evo_parent_ic50.xlsx',
                            index_col=1, skiprows=1)['Parent mean IC50 [log2(μg/mL)]']
...
def plot_time_series(strain, stress, color, start_day=start_day, zorder=1, alpha=0.15, marker='.', i=None, print_title=True, label=None):
    """
    ...
    if strain == 26:
        ...
        plt.plot(np.linspace(start_day, traj_len, traj_len - start_day + 1),
        ...
        plt.plot(t, trajectory.iloc[start_day-1:],
                 color=color, alpha=alpha, zorder=zorder, marker=marker, label=label)
    if i == 0 and print_title:
        plt.title(title, fontsize=15)


def plot_strain(strain_num_start, strain_num_end, stress1, stress2, strain_label, roll_win=None, cmap_name=None, cmap_level=6):
    for strain in range(strain_num_start, strain_num_end+1):
        ...
        df = pd.read_csv('./data/trajectories/strain' +
                         str(strain)+'.csv', index_col=0)
        trajectory1 = df.loc[stress1]
        trajectory2 = df.loc[stress2]
        if roll_win is not None:
            trajectory1 = trajectory1.T.rolling(
                roll_win, min_periods=1, win_type='triang').mean()
            trajectory2 = trajectory2.T.rolling(
                roll_win, min_periods=1, win_type='triang').mean()
        if strain == strain_num_end:
            plt.plot(trajectory1.values, trajectory2.values, '.-',
                 lw=0.9, markersize=4.8, color=cmap2[strain-strain_num_start+2], label=strain_label, zorder=3)
        else:
            plt.plot(trajectory1.values, trajectory2.values, '.-',
                 lw=0.9, markersize=4.8, color=cmap2[strain-strain_num_start+2], zorder=3)
        #plt.scatter(trajectory1[0], trajectory2[0], color=cmap2[strain-strain_num_start+2], zorder=2)
